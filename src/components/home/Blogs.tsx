...
export default function Blogs() {
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/v2/pages/?type=landing.SEOLandingPage&fields=hero_title,hero_subtitle,hero_image&limit=15`)
      .then((r) => r.json())
      ...
      <div className="px-6 sm:px-12 lg:px-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10 text-center md:text-left">
          <div className="flex-1">
            <h2 className="text-2xl sm:text-3xl font-normal text-[#318bb0] tracking-tight font-serif">
              Guides & Resources
            </h2>
        ...
        <div className="flex md:block md:columns-3 lg:columns-5 gap-6 overflow-x-auto md:overflow-x-visible snap-x snap-mandatory scrollbar-none pb-6 -mx-6 px-6 md:mx-0 md:px-0">
          {pages.map((page, idx) => {
            const image = page.hero_image?.url || FALLBACK_IMAGES[idx % FALLBACK_IMAGES.length]
            const title = page.hero_title || page.title
            const excerpt = page.hero_subtitle || ''
