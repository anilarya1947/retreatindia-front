...
export default function Footer() {
  const citiesCol1 = ['Delhi', 'Mumbai', 'Kolkata', 'Bangalore', 'Chennai', 'Hyderabad']
  const citiesCol2 = ['Ahmedabad', 'Chandigarh', 'Pune', 'Surat', 'Jaipur', 'Lucknow']
  const citiesCol3 = ['Nagpur', 'States & UTs', 'All Cities']
  ...
  return (
    <footer className="relative text-white rounded-t-[32px] sm:rounded-t-[40px] pt-12 pb-10 sm:pt-16 sm:pb-12 overflow-hidden">
      {/* Background image */}
      <div
        className="absolute inset-0 pointer-events-none z-0"
        style={{
          backgroundImage: "url('/images/homepage/footebg.png')",
      ...
      {/* ----------------- DESKTOP VIEW ----------------- */}
      <div className="hidden md:block relative z-10 px-6 sm:px-12 lg:px-12">
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8 pb-12 border-b border-white/20 mb-12">
          ...
          <div className="flex items-center gap-3 border-r-[1px] border-dotted border-white pr-8">
            <div className="flex flex-col leading-none">
              <img src="/images/homepage/recoverindia-logo-wht.png" alt="RecoverIndia Logo" />
            </div>
          ...
          <div className="flex items-center gap-3 border-r-[1px] border-dotted border-white pr-8">
            <div className="flex flex-col leading-none">
              <img src="/images/homepage/anvaya-logo-wht.png" alt="Anvaya Logo" />
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8 mb-12">
          ...
          <div>
            ...
            <div className="mb-6">
              ...
              <ul className="space-y-2 text-sm font-medium text-white/80">
                {adultsList.map(item => (
                  <li key={item}><Link href="#search" className="hover:underline">{item}</Link></li>
                ))}
              </ul>
            </div>
            <div>
          ...
          <div>
            <div className="mb-8">
              ...
              <ul className="space-y-2 text-sm font-semibold text-white/90">
                {guideList.map(item => (
                  <li key={item}><Link href="#faqs" className="hover:underline">{item}</Link></li>
                ))}
              </ul>
            </div>
            <div>
      ...
      <div className="block md:hidden relative z-10 px-3 md:px-6">
        <div className="flex flex-col gap-10">
          ...
          <div>
            <p className="text-sm font-bold text-[#e6b571] uppercase tracking-wider mb-3">Rehabs By Location</p>
            <div className="grid grid-cols-3 gap-y-2.5 gap-x-1.5 text-xs font-semibold text-white/90">
              <div className="flex flex-col gap-2">
                {citiesCol1.map(c => <Link key={c} href="#search" className="hover:underline">{c}</Link>)}
              </div>
              <div className="flex flex-col gap-2">
                {citiesCol2.map(c => <Link key={c} href="#search" className="hover:underline">{c}</Link>)}
              </div>
              <div className="flex flex-col gap-2">
                {citiesCol3.map(c => <Link key={c} href="#search" className="hover:underline">{c}</Link>)}
              </div>
            </div>
          </div>

          {/* Rehabs by Treatment (Screenshot 2 & 3) */}
          <div>
            <p className="text-sm font-bold text-[#e6b571] uppercase tracking-wider mb-3">Rehabs By Treatment</p>
            <div className="grid grid-cols-2 gap-x-4 gap-y-6 text-xs font-medium text-white/80">
              {/* Col 1 */}
              <div>
                <p className="font-extrabold text-white uppercase mb-2">Adults</p>
                <div className="flex flex-col gap-2.5">
                  {adultsList.slice(0, 6).map(item => (
                    <Link key={item} href="#search" className="hover:underline">{item}</Link>
                  ))}
                </div>
              </div>
              {/* Col 2 */}
              <div>
                <p className="font-extrabold text-white uppercase mb-2">Geriatric (Elderly)</p>
                <div className="flex flex-col gap-2.5">
                  {geriatricList.map(item => (
                    <Link key={item} href="#search" className="hover:underline">{item}</Link>
                  ))}
                ...
                <p className="font-extrabold text-white uppercase mt-6 mb-2">Special & Emerging</p>
                <div className="flex flex-col gap-2.5">
                  {specialList.slice(0, 4).map(item => (
                    <Link key={item} href="#search" className="hover:underline">{item}</Link>
                  ))}
                </div>
              </div>
            ...
            {/* Child & Adolescents list (Screenshot 3 top left) */}
            <div className="mt-6 text-xs font-medium text-white/80">
              <p className="font-extrabold text-white uppercase mb-2">Child & Adolescents</p>
          ...
          {/* Guide & Resources (Screenshot 3) */}
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-sm font-bold text-[#e6b571] uppercase tracking-wider mb-3">Guide</p>
              <div className="flex flex-col gap-2 text-xs font-semibold text-white/90">
                {guideList.map(item => (
                  <Link key={item} href="#faqs" className="hover:underline">{item}</Link>
                ))}
              </div>
            </div>
            <div>
              <p className="text-sm font-bold text-[#e6b571] uppercase tracking-wider mb-3">Resources</p>
              <div className="flex flex-col gap-2 text-xs font-semibold text-white/90">
                {resourcesList.map(item => (
                  <Link key={item} href="#faqs" className="hover:underline">{item}</Link>
                ))}
          ...
          {/* Logo & Social Icons (Screenshot 3 bottom) */}
          <div className="border-t border-white/15 pt-6 mt-2 flex flex-col gap-5">
            <div className="flex items-center gap-3">
              <img src="/images/homepage/anvaya-logo-wht.png" alt="Anvaya Logo" className="h-10 object-contain shrink-0" />
              <div className="text-[11px] leading-tight text-white/80 font-medium">
                Our team of <br />Expert Psychiatrists & <br />Psychologists
              </div>
            </div>
